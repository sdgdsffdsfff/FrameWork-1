...
function page_save()
{
	$("#aform").attr("action","opiniontemplate_update.action");
...
<body>
	...
	<div id="pageContainer">
		...
		<input type="hidden" id="id" name="id" value="${data.ot.id}">
		<table class="formGrid">
   			 <tr id="repeatedTask">
				<td class="r">意见分类：</td>
				<td>
					<span class="selectSpan"><input type="hidden" id ="cclass" name="cclass" />
						<input class="select" data-options="公共||个人" data-values="public||private" data-default="${data.ot.cclass}" style="width:10em"/>
					</span>
				</td>
			</tr>
   			 <tr>
		    	<td class="r" ><label for="">意见内容：</label></td>
		    	<td><textarea class="required" id= "opinion" name="opinion" maxlength='500' rows="5">${data.ot.opinion}</textarea></td>
   			 </tr> 
		</table>
		</form>
	</div>
