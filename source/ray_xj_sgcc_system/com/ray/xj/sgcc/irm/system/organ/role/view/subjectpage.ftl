...
<input id="result" name="result" type="hidden" value="" />
	...
	<tbody>
		<!-- 显示数据-->
		<#list data.page.result as subject>
			<tr>
				<td>
					...
					<#list data.lists as rolesubject>
						<#if rolesubject.subjectid == subject.id>
							<#assign aclass = " checkboxChecked">
							<#assign check = "1">
						</#if>
					</#list>
					<input data-id="${subject.id}" class="checkbox ${aclass}" name="index" value="${check}">
					<input type="hidden" name="osubjectid" value="${subject.id}">
...
<script type="text/javascript">
var navigationJSON=[ {name:'系统管理',link:'${base}/module/irm/portal/portal/portal/portal_browse.action?ccate=admin'},{name:'角色',link:'${base}/module/irm/system/organ/role/role_browse.action?_searchname=system.organ.role.browse'}, {name:'角色栏目'}];
...
function page_save()
{
	...
	{
		checkbox();
		$("#dataform").attr("action","${base}/module/irm/system/organ/role/role_savesubject.action");
