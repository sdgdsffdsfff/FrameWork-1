...
<div class="oTable">
	<table class="dataGrid">
		...
		<tbody>
		<!-- 显示数据-->
		<#list apage.result as portalitem>
			<tr>
				<td>
					<#assign aclass = "">
					<#assign check = "0">
					<#list data.lists as roleportalitem>
						<#if roleportalitem.portalitemid == portalitem.id>
							<#assign aclass = " checkboxChecked">
							<#assign check = "1">
						</#if>
					</#list>
					<input data-id="${portalitem.id}" class="checkbox ${aclass}" name="index" value="${check}">
					<input type="hidden" name="oportalitemid" value="${portalitem.id}">
					<input type="hidden" name="portalitemid" value="${portalitem.id}">					
				</td>
				<td>${portalitem.cname}</td> 
				<td>${portalitem.ccate}</td> 
				<td>${portalitem.ctype}</td> 
			</tr>
		</#list>
		</tbody>
	</table>
...
<script type="text/javascript">
var navigationJSON=[ {name:'系统管理',link:'${base}/module/irm/portal/portal/portal/portal_browse.action?ccate=admin'},{name:'角色',link:'${base}/module/irm/system/organ/role/role_browse.action?_searchname=system.organ.role.browse'}, {name:'角色门户菜单'}];
...
function page_save()
{
	...
	{
		checkbox();
		$("#dataform").attr("action","${base}/module/irm/system/organ/role/role_saveportalitem.action");
