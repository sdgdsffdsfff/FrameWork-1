...
$('#bt_confirm').click(function(){
	...
	}else
	{
		...
		if(businesstype == "change")
		{
			 if(ctype == "send")
    	    {
    	    	if(state == "costconfirm")
    	    	{
    				$("#form").attr("action","${base}/module/irm/config/change/configchange/configchange_sendcomment.action");
	 	    		$("#form").trigger('submit');
	 	    	}
	 	    	else
	 	    	{
	 	    		//alert("将变更单特送给"+$('#dusername').val());
    	...
    	else if(businesstype == "businessact")
    	{
    		alert("将业务活动送记录审核给"+$('#dusername').val()+"。");
    		$("#form").attr("action","${base}/module/irm/business/businessact/businessact/businessact_forward.action");
    		$("#form").trigger('submit');
    	}
...
<body id="layoutPage">
<div id="fixedOp">
<#if "${arg.businesstype}" == "change" && "${arg.adescription}" != "">
...
<input type="hidden" id="backurl" name="backurl" value="${base}/module/irm/config/change/configchange/configchange_readpageframe.action?id=${arg.id}">
<#elseif "${arg.businesstype}" == "project">
<input type="hidden" id="backurl" name="backurl" value="${base}/module/irm/project/project/project/project_readpageframe.action?id=${arg.id}">
<#elseif "${arg.businesstype}" == "task">
<input type="hidden" id="backurl" name="backurl" value="${base}/module/irm/project/task/task/task_readpageframe.action?id=${arg.id}">
<#elseif "${arg.businesstype}" == "businessact">
<input type="hidden" id="backurl" name="backurl" value="${base}/module/irm/business/businessact/businessact/businessact_locate.action?viewtype=todo&id=${arg.id}">
</#if>
</form>
...
<#if arg.choose == 'role'>
	<li><iframe src="${base}/module/irm/system/flow/flowlog/flowlog_rolepage.action?rolename=${arg.rolename}" frameborder="0"></iframe></li>
</#if>
<#if arg.choose == 'user'>
