...
//初始化根节点
//$('#leftTreeDiv').append('<ul id="gAjaxTree">'+ajax4Tree_pz1('shortcut_tree.action',rootID)+'</ul>');
...
$('#gAjaxTree li span').live('click',function(){
    ...
    if(obj.find('ul').length==0){
		if(obj.attr('data-leaf')!=0){//如果不是终点 才 ajax
			obj.append('<ul>'+ajax4Tree_pz1('shortcut_treechild.action',obj.attr('data-id'))+'</ul>')
		}
    }else{
	...
	window.rightIframe.location='shortcut_browse.action?_searchname=system.portal.shortcut.browse&supid='+obj.attr('data-id');	
...
<ul id="gAjaxTree">
	<li data-pid="R0" data-index="1" data-parent="" data-leaf="1" data-id="R0"><span class="root">快捷项</span></li>
