...
<table width="100%" border="0" cellpadding="0" cellspacing="1"   id="listtable">  
	  <tr>
       ...
       <tr name="trs" id="${options_index}"  class="hand" bgcolor="<#if options_index % 2 == 0>F4F4F4<#else>FDFDFD</#if>" onclick="select(${options_index})">     
            <td  align="left"><input type="radio" name="radios" id="radio_${options_index}"/>${options.searchlinkid}<input type="hidden" id="oorder" name="oorder"  value="${options_index+1}"></td> 
            <td>${options.field} <input type="hidden" id="searchlinkid" name="searchlinkid" value="${options.searchlinkid}"  ></td>   
            <td>${options.title} </td> 
