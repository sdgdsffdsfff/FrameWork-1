...
<#macro displayurl vo>
  	...
  	<#if url.visible == "1">
  	<button id="bt_${url.pname}">${url.title}</button>
  	</#if>
  	</#list>
</#macro>
		...
		<#list vo.searchitems as searchitem>
			...
			<#assign value = "arg.${searchitem.htmlfield}"?eval>
				...
				<#assign vnum = vnum + 1>
					...
					<td>&nbsp;<@displayaitem option = searchitem value = "${value}" />
					</td>
				</#if>
			</#if>
		</#list>
...
<@compress>
	...
	<input type="text" class="text" id="${option.htmlfield}" name="${option.htmlfield}" size="35"  dataType="${option.required}" value="<@displayfieldvalue option = option value = encode_html_value(value) />" <#if option.readonly == "Y">readonly</#if>>
	<#elseif option.edittype == "textint">
	<input type="text" class="text int" id="${option.htmlfield}" name="${option.htmlfield}" size="35"  dataType="${option.required}" value="<@displayfieldvalue option = option value = encode_html_value(value) />" <#if option.readonly == "Y">readonly</#if>>
	...
	<#--
	<input class="text ajax" type="text" id="${option.htmlfield}" name="${option.htmlfield}" size="35"  dataType="${option.required}" value="<@displayfieldvalue option = option value = encode_html_value(value) />" <#if option.readonly == "Y">readonly</#if>>
	-->
	<input class="text ajax" type="text" id="${option.htmlfield}" name="${option.htmlfield}" size="35"  dataType="${option.required}" value="<@displayfieldvalue option = option value = encode_html_value(value) />" <#if option.readonly == "Y">readonly</#if> ${option.param1}>
	
	<#elseif option.edittype == "dateinput">
	<input class="date <#if option.required=='Y'>required</#if>" type="text" id="${option.htmlfield}" name="${option.htmlfield}" size="35"  dataType="${option.required}" value="<@displayfieldvalue option = option value = encode_html_value(value) />" <#if option.readonly == "Y">readonly</#if>>
	<#elseif option.edittype == "timeinput">
	<input class="date <#if option.required=='Y'>required</#if>" type="text" id="${option.htmlfield}" name="${option.htmlfield}" size="35"  dataType="${option.required}" value="<@displayfieldvalue option = option value = encode_html_value(value) />" <#if option.readonly == "Y">readonly</#if>>
	<#elseif option.edittype == "reqopenwin">
	<input class="text required openwin" id="${option.htmlfield}" name="${option.htmlfield}" size="35"  dataType="${option.required}"  value="<@displayfieldvalue option = option value = encode_html_value(value) />" <#if option.readonly == "Y">readonly</#if>>
	</#if>
	...
	<a href="javascript:void(0)" onclick="openwin('${base}/module/app/system/query/search/searchitem_locate.action?_searchname=searchitem.locate&searchitemid=${option.searchitemid}',pub_width_mid, pub_height_mid)">定义</a>
...
<tr>
<#if vo.ischeck == "Y">
<th width="1%" nowrap><nobr><input class="checkbox" onclick="" name="indexall" id="indexall" /></nobr></th>
</#if>
...
<#list vo.searchoptions as option>
	...
	<#else>
	<th align="center" width="${option.displaywidth}%" nowrap >${option.title}<span style="font-family:Webdings; font-size:9px; font-weight:normal"></span>
	<#-- 配置管理使用 -->
	<#if (Session.sys_login_token)??>
	<#if Session.sys_login_token.sys_login_user=="admin">
	<a href="javascript:void(0)" onclick="openwin('${base}/module/app/system/query/search/searchoption_locate.action?_searchname=searchoption.locate&searchoptionid=${option.searchoptionid}',pub_width_mid, pub_height_mid)">定义</a>	
...
<#macro displayheaderjson vo arg>
<#assign num = 0>
<#assign num_index = 0>
<#assign data="[">
	<#if vo.ischeck == "Y">
	<#assign data= data+ "{isCheckbox:true},">
	</#if>
	<#list vo.searchoptions as option>
	<#assign num = num +1>
	<#if option.visible == "1">
	...
	<#assign num_index = num_index + 1>
	<#assign data= data+ "{name:'${option.title}',valuewidth:'${option.valuewidth}',orderby:'${option.field}'}">
	</#if>
	</#list>
<#assign data= data+ "]">
${data}
...
<#macro displaylistjson apage vo>
<#assign keyvalue = "">
<#assign keyname = "">
<#assign data="[">
<#list apage.result as hs>
	...
	<#list vo.searchoptions as searchoption>
		<#if searchoption.pkey == "1">
			<#assign keyvalue =hs[searchoption.field]>
			<#assign keyname =searchoption.htmlfield>
		</#if>
	</#list>
	<#if vo.ischeck == "Y">
	<#assign data= data+ "{isCheckbox:true,id:'${keyvalue}'},">
	</#if>
	<#list vo.searchoptions as option>
	<#if option.visible == "1">
	<#assign value = "hs.${option.field}"?eval>
	<#escape value as value!""></#escape>
	<#assign value><@displayfield option = option value = value /></#assign>
	<#if num_index !=0>
	<#assign data= data+ ",">
	</#if>
	<#assign num_index = num_index + 1>
	<#assign value = value?replace("'","&apos;")>
	<#assign value = value?replace("''","&quto;")>
	<#assign value = value?replace("\r","\n")>
	<#assign value = value?replace("\n","<br>")>
	<#assign data= data+ "{name:'${value}'">
	<#if option.url !="">
	<#assign data= data+ ",link:'${base}${option.url}&${keyname}=${keyvalue}'">
	</#if>
	<#assign data= data+ "}">
	</#if>
	</#list>
	
	<#if hs_index+1 lt apage.result?size>
	<#assign data= data+ "],">
	<#else>
	<#assign data= data+ "]">
	</#if>
...
<#assign pagesize = apage.pagesize>
...
<#if apage.maxpage &gt; 1>
	...
	<td width="30">每页显示<input id="jumppagesize" name="jumppagesize" size="3" value="${apage.pagesize?default(15)}">条记录</td>
	<td>&nbsp;</td>
</#if>	
...
<#macro displaynavigationjson apage vo arg>
<#assign urldate = "">
<#list vo.searchitems as searchitem>
		<#assign value = "arg.${searchitem.htmlfield}"?eval>
		<#assign urldata = urldata + "&${searchitem.htmlfield}="+value>
</#list>
{current:${apage.currentpage},items:${apage.rowcount},pages:${apage.maxpage},step:${apage.pagesize},url:'${base}${vo.formaction}${urldata}',orderby:'${apage.sortfield}',order:'${apage.sorttag}'}	
</#macro>
...
<@displayfieldvalue option = option value = value />
<#elseif option.edittype == "dictionary">
<@displayfieldvalue_dictionary option = option value = value />
</#if>
</@compress>
</#macro>
...
<@compress>
	<#if option.visible == "1">
		<#if option.edittype == "text">
		<input type="text" class="text <#if option.required=='Y'>required</#if>" id="${option.htmlfield}" name="${option.htmlfield}" style="width:<#if option.displaywidth??>${option.displaywidth}<#else>20em</#if>;" value="<@displayfieldvalue option = option value = encode_html_value(value) />" <#if option.readonly == "Y">readonly</#if>>
		<#elseif option.edittype == "textint">
		<input type="text" class="text int <#if option.required=='Y'>required</#if>" id="${option.htmlfield}" name="${option.htmlfield}" style="width:<#if option.displaywidth??>${option.displaywidth}<#else>20em</#if>;" value="<@displayfieldvalue option = option value = encode_html_value(value) />" <#if option.readonly == "Y">readonly</#if>>
		<#elseif option.edittype == "password">
		<input type="password" class="text <#if option.required=='Y'>required</#if>" id="${option.htmlfield}" name="${option.htmlfield}" style="width:<#if option.displaywidth??>${option.displaywidth}<#else>20em</#if>;" value="">
		<#elseif option.edittype == "radio">
		<@displayafield_radio option = option value = value />
		<#elseif option.edittype == "select">
		<@displayafield_select option = option value = value />
		<#elseif option.edittype == "textarea">
		...
		<#elseif option.edittype == "ajaxinput">
		<input class="text ajax <#if option.required=='Y'>required</#if>" type="text" id="${option.htmlfield}" name="${option.htmlfield}" style="width:<#if option.displaywidth??>${option.displaywidth}<#else>20em</#if>;"  value="<@displayfieldvalue option = option value = encode_html_value(value) />" <#if option.readonly == "Y">readonly</#if> ${option.param1}>
		<#elseif option.edittype == "dateinput">
		<input class="date <#if option.required=='Y'>required</#if>" id="${option.htmlfield}" name="${option.htmlfield}" style="width:<#if option.displaywidth??>${option.displaywidth}<#else>20em</#if>;"  value="<@displayfieldvalue option = option value = encode_html_value(value) />" <#if option.readonly == "Y">readonly</#if>>
		<#elseif option.edittype == "reqopenwin">
		<input class="text <#if option.required=='Y'>required</#if> openwin" id="${option.htmlfield}" name="${option.htmlfield}" style="width:<#if option.displaywidth??>${option.displaywidth}<#else>20em</#if>;"  value="<@displayfieldvalue option = option value = encode_html_value(value) />" <#if option.readonly == "Y">readonly</#if>>
		...
		<#elseif option.edittype == "timeinput">
		<input class="datetime <#if option.required=='Y'>required</#if>" name="${option.htmlfield}" id="${option.htmlfield}" value="<@displayfieldvalue option = option value = encode_html_value(value)/>">
		<#elseif option.edittype == "timeinputnow">
		<input class="datetime now <#if option.required=='Y'>required</#if>" name="${option.htmlfield}" id="${option.htmlfield}" value="<@displayfieldvalue option = option value = encode_html_value(value)/>">
		<#elseif option.edittype == "dateinputnow">
		<input class="date now <#if option.required=='Y'>required</#if>" name="${option.htmlfield}" id="${option.htmlfield}" value="<@displayfieldvalue option = option value = encode_html_value(value)/>">
		</#if>
	</#if>
...
<#macro displayafield_radio option value>
	<span class="allRadios">
		<input type="hidden" name="${option.htmlfield}" value="${value}"/>
		...
		<label><input class="radio <#if value ==aoption.cvalue>radioChecked</#if>" value="${aoption.cvalue}">${aoption.ctext}</label>
		</#list>
		<#else>
		<#assign onames = option.param1.split(",")>
		<#assign ovalues = option.param2.split(",")>
		<#list onames as oname>
		<label><input class="radio <#if value ==ovalues[oname_index]>radioChecked</#if>" value="${ovalues[oname_index]}">${oname}</label>
		</#list>
		</#if>
	</span>
</#macro>
...
<#macro displayafield_select option value>
	<select class="text" id="${option.htmlfield}" name="${option.htmlfield}" dataType="${option.required}">
		<option value="">&nbsp;&nbsp;</opiton>
			...
			<#if option.dstype == "sql">
				<#assign options = stack.findValue("@com.ray.app.query.function.view.SQLSelect@get_data(\"${option.param1}\")") >
					...
					<#else>
						...
						<input type="hidden" id="${option.htmlfield}" name="${option.htmlfield}" value="${value}" />
						<input class="select <#if option.required=='Y'>required</#if>" style="width:<#if option.displaywidth??>${option.displaywidth}<#else>110px</#if>;" data-options="<#if finaltext!="">${finaltext}</#if>" data-values="<#if finalvalue!="">${finalvalue}</#if>" data-default="${value}">
						</span>
					</#if>
			<#else>
				<#assign onames = option.param1.split(",")>
				...
				<#list onames as oname>
					<#if finalvalue =="">
					<#assign finalvalue = ovalues[oname_index]>
					<#assign finaltext = onames[oname_index]>
					<#else>
			...
			<#if option.dstype == "sql">
				<#assign options = stack.findValue("@com.ray.app.query.function.view.SQLSelect@get_data(\"${option.param1}\")") >
					...
					<#else>
						...
						<input class="select2 <#if option.required=='Y'>required</#if>" style="width:<#if option.displaywidth??>${option.displaywidth}<#else>20em</#if>;" id="${option.htmlfield}" name="${option.htmlfield}" data-options="<#if finaltext!="">${finaltext}</#if>" data-values="<#if finalvalue!="">${finalvalue}</#if>" data-selected="${selectnum}" value="${value}">
					</#if>
			<#else>
				<#assign onames = option.param1.split(",")>
				<#assign ovalues = option.param2.split(",")>
				...
				<input class="select2" id="${option.htmlfield}" name="${option.htmlfield}" style="width:<#if option.displaywidth??>${option.displaywidth}<#else>20em</#if>;" data-options="<#if finaltext!="">${finaltext}</#if>" data-values="<#if finalvalue!="">${finalvalue}</#if>" data-selected="${selectnum}" value="${value}">							
			</#if>
...
<#macro displayfieldvalue_dictionary option value>
	<@compress>
	<#assign sql = "${option.param1?replace(':value','${value}')}">
	<#assign options = stack.findValue("@com.ray.app.query.function.view.SQLSelect@get_data(\"${sql}\")") >
