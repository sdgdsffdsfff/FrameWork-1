...
<#list apage.result as hs>
	...
	<input type="radio" id="sindex_${hs_index +1}" name="sindex" value="0" onclick="page_radio_row_click(this, '${keynames}', '${keyvalues}', ${hs_index + 1})">
	<input type="hidden" id="index_${hs_index + 1}" name="index" value="${keyvalues}">
...
<@compress>
	...
	<#if Session["SPRING_SECURITY_CONTEXT"]??>
	<#if Session["SPRING_SECURITY_CONTEXT"].authentication.principal.username=="admin">
	<a href="javascript:void(0)" onclick="openwin('${base}/module/app/system/query/search/searchoption_locate.action?_searchname=searchoption.locate&searchoptionid=${option.searchoptionid}',pub_width_small, pub_height_small)">定义</a>	
	</#if>
	</#if>
</@compress>
