...
<form id="locateform" method="post" action="${base}/${vo.formaction}">
<input type="hidden" id="_searchname" name="_searchname" value="">
<#list vo.searchoptions as option>
<#if option.edittype == "hidden">
<input type="hidden" id="${option.htmlfield}" name="${option.htmlfield}" value="${aobj[option.field]}">
</#if>
</#list>
...
<#assign vnum = vnum + 1>
	...
	<#if option.edittype = "textarea">
		...
		<td colspan="3"><@pub_macros.displayafield option = option value = "${value}" /></td>
		<#assign vnum = vnum + 1>
	<#else>
		<td class="r"><label for="${option.htmlfield}">${option.title}：</label></td>
		<td><@pub_macros.displayafield option = option value = "${value}" /></td>
	</#if>
