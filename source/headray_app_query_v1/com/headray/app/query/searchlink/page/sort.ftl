...
<div id="navigation"><span id="op"></span><strong>当前位置：</strong>管理平台.基础数据管理.查询信息管理.查询链接.排序</div> 
...
<table width="100%" border="0" cellpadding="0" cellspacing="1"   id="listtable">  
	  <tr>
       ...
       <span id="trIds">
       <#list data.links as links>  
       <tr name="trs" id="${links_index}" class="hand" bgcolor="<#if links_index % 2 == 0>F4F4F4<#else>FDFDFD</#if>" onclick="select(${links_index})">    
            <td  align="left"><input type="radio" name="radios" id="radio_${links_index}"/>${links.searchlinkid}<input type="hidden" id="oorder" name="oorder"  value="${links_index+1}"></td> 
            <td>${links.linksearchname} <input type="hidden" id="searchlinkid" name="searchlinkid" value="${links.searchlinkid}"  ></td>   
