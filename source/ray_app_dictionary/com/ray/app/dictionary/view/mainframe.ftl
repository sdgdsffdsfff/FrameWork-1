...
//初始化根节点
//$('#leftTreeDiv').append('<ul id="gAjaxTree">'+ajax4Tree_pz1('${base}/module/app/system/dictionary/dictionary/dictionary_tree.action',rootID)+'</ul>');
...
$('#gAjaxTree li span').live('click',function(){
    ...
    if(obj.find('ul').length==0){
		if(obj.attr('data-leaf')!=0){//如果不是终点 才 ajax
			obj.append('<ul>'+ajax4Tree_pz1('${base}/module/app/system/dictionary/dictionary/dictionary_treechild.action',obj.attr('data-id'))+'</ul>')
		}
    }else{
        obj.find('ul').toggle();	
    }
	...
	window.rightIframe.location='${base}/module/app/system/dictionary/dictionary/dictionary_browse.action?_searchname=system.dictionary.dictionary.browse&dclassid='+obj.attr('data-id');	
