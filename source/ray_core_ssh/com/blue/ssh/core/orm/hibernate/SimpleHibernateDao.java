 ...
 * 
 * $Id: SimpleHibernateDao.java,v 1.1 2012/03/13 04:05:42 yanchangyong Exp $
 */
package com.blue.ssh.core.orm.hibernate;
...
import com.blue.ssh.core.utils.reflection.ReflectionUtils;
...
public class SimpleHibernateDao<T, PK extends Serializable>
{
	...
	/**
	 * 按属性查找对象列表, 匹配方式为相等, 增加排序功能.
	 */
	public List<T> findBy(final String propertyName, final Object value, Order order)
	{
		Assert.hasText(propertyName, "propertyName不能为空");
		Criterion criterion = Restrictions.eq(propertyName, value);
	...
	public <X> X findUnique(final String hql, final Object... values)
	{
		...
		catch(Exception e)
		{
			...
			try
			{
				return (X) entityClass.newInstance();
			}
			catch(Exception ex)
			{
				logger.error(e.getMessage());
	...
	/*
	// 蒲剑 添加动态对象查询方式，封装异常处理
	public Map findUniqueDyObj(final Criterion... criterions)
	{
		Map obj = new HashMap();
		try
		{
			obj = BeanUtils.describe(createCriteria(criterions).uniqueResult());
		}
		catch (Exception e)
		{
	...
	public Map findUniqueDyObj(final String hql, final Map<String, ?> values)
	{
		Map obj = new HashMap();
		try
		{
			obj = BeanUtils.describe(createQuery(hql, values).uniqueResult());
		}
		catch (Exception e)
		{
			logger.debug(e.getMessage());
	...
	public Map findUniqueDyObj(final String hql, final Object... values)
	{
		Map obj = new HashMap();
		try
		{
	...
	public Map findUniqueByDyObj(final String propertyName, final Object value)
	{
		Map obj = new HashMap();
		try
		{
			Assert.hasText(propertyName, "propertyName不能为空");
			Criterion criterion = Restrictions.eq(propertyName, value);
			obj = BeanUtils.describe(createCriteria(criterion).uniqueResult());
		}
		catch (Exception e)
		{
			logger.debug(e.getMessage());
	...
	public Map getDyObj(final PK id)
	{
		Map obj = new HashMap();
		try
		{
			Assert.notNull(id, "id不能为空");
			obj = BeanUtils.describe(getSession().load(entityClass, id));
		}
		catch (Exception e)
		{
			logger.debug(e.getMessage());
